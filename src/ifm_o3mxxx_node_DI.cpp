...
#include "ifm/DI/example_results.h"
#include "ifm/DI/example_types.h"
#include "ifm_o3mxxx/ifm_o3mxxx_pointcloud_DI.h"
#include "ifm_o3mxxx/ifm_o3mxxx_basicfunctions_DI.h"
...
int processChannel20_DI(void* buf, uint32_t size)
{
    ifm_o3m_AlgoIFOutput_DIA1_1_0* pDIA1_1_0;
    ifm_o3m_AlgoIFOutput_DIA1_1_1* pDIA1_1_1;
    ifm_o3m_AlgoIFOutput_DIA1_1_2* pDIA1_1_2;
...
int main(int argc, char **argv)
{
    ...
    node_priv.param("BasicFunctions", out_ifm_bf_results, std::string("ifm_o3mxxx_bf_results"));
    ...
    ros::Publisher pub_ifm_raw = node.advertise<ifm_o3mxxx::ifm_o3mxxx_pointcloud_DI> (out_ifm_raw, 1);
    ros::Publisher pub_ifm_bf_results = node.advertise<ifm_o3mxxx::ifm_o3mxxx_basicfunctions_DI> (out_ifm_bf_results, 1);

    UDPReceiver receiver;
    receiver.init(port, ip);

    // The data is in the channel 8
    const uint32_t pointCloudDataChannel = 8;
    const uint32_t functionDataChannel = 20;
    ...
    while (node.ok())
    {
        ...
        if(ret_val >= 0){
            ...
            if (!(ph->ChannelID == pointCloudDataChannel) || (ph->ChannelID == functionDataChannel))
            {
                //printf("Sorry, Support for channel %d not implemented yet.\n", ph->ChannelID);
                continue;
            }
            ...
            if (startOfChannelFound)
            {
                ...
                if (ph->IndexOfPacketInChannel == ph->NumberOfPacketsInChannel -1)
                {
                    if(ph->ChannelID == pointCloudDataChannel)
                    {
